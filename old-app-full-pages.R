...
ui <- navbarPage(
  ...
  # LANDING PAGE IS BUDGET SUMMARY 
  tabPanel("Summary of Planned Interventions and Costs", 
           h4("This page provides a summary of the costs associated with the 2025 prioritised intervention package at the National and State level. First select a spatial level of interest to initiate summaries "),
           sidebarLayout(
             sidebarPanel(
               # Dropdown for selecting spatial level
               selectInput(
                 inputId = "spatial_level",
                 label = "Select Spatial Level:",
                 choices = c("", "National", "State"),
                 selected = "",
                 multiple = FALSE
               ),
               # Conditional panel to display state selection only if "State" is selected
               conditionalPanel(
                 condition = "input.spatial_level == 'State'",
                 selectizeInput(
                   inputId = "state",
                   label = "Select State of Interest:",
             ...
             mainPanel(
               width = 12,
               fluidRow(
                 column(6, leafletOutput("map1", height = "550px")),
                 column(6, leafletOutput("map_interactive", height = "550px"))
               ),
...
server <- function(input, output, session) {
  ...
  # Reactively manage the spatial level and state selection
  selected_content <- reactive({
    req(input$spatial_level)  # Ensure spatial level is selected
    if (input$spatial_level == "National") {
      list(level = "National", state = NULL)
    } else if (input$spatial_level == "State") {
      req(input$state)  # Ensure a state is selected
      list(level = "State", state = input$state)
    }
  })
  
  # Observe the Clear Selection button and reset inputs
  observeEvent(input$clear_selection, {
    updateSelectInput(session, "spatial_level", selected = "")
    updateSelectizeInput(session, "state", selected = "")
    updateRadioButtons(session, "currency_option", selected = "USD")
  })
  
  #--SUMMARY TAB-------------------------------------------------------------------------------
  
  # Reactive: Manage spatial level and state selection
  selected_content <- reactive({
    req(input$spatial_level)  # Ensure spatial level is selected
    if (input$spatial_level == "National") {
      list(level = "National", state = NULL)
  ...
  # Render the intervention mix map
  output$map1 <- renderLeaflet({
    content <- selected_content()  # Reactive input
    req(content$level == "National" || !is.null(content$state))  # Ensure a valid selection
  ...
  # Render the interactive intervention map
  ...
  # Observer: Update the interactive map based on selected interventions
  observeEvent(input$selected_interventions, {
    selected_interventions <- input$selected_interventions
    
    req(selected_interventions)  # Ensure selections are available
  ...
  # Render the dynamic info section
